...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, opts) {
	...
	return () => {
		...
		// Extensions removed as user doesn't want extensions
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const tasks = [
			// compileNativeExtensionsBuildTask, // Removed as user doesn't want extensions
		...
		const vscodeTask = task.define(`vscode${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(
			...
			// Extension compilation tasks removed as user doesn't want extensions
...
gulp.task(task.define(
	...
	task.series(
		coreCI,
		// compileAllExtensionsBuildTask, // Removed as user doesn't want extensions
		function () {
			const pathToMetadata = './out-build/nls.metadata.json';
			// const pathToExtensions = '.build/extensions/*'; // Removed as user doesn't want extensions
			const pathToSetup = 'build/win32/i18n/messages.en.isl';

			return es.merge(
				gulp.src(pathToMetadata).pipe(i18n.createXlfFilesForCoreBundle()),
				gulp.src(pathToSetup).pipe(i18n.createXlfFilesForIsl())
				// gulp.src(pathToExtensions).pipe(i18n.createXlfFilesForExtensions()) // Removed as user doesn't want extensions
